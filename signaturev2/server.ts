...
async function startServer() {
  ...
  // API Route to check if SMTP is configured in env variables
  ...
  app.post("/api/send-email", async (req, res) => {
    const { to, subject, code, companyName, clientName, totalAmount, details, smtpConfig, appUrl } = req.body;

    if (!to) {
      return res.status(400).json({ error: "L'adresse e-mail du destinataire est requise." });
    }

    // SMTP environment variable detection (or dynamic config loaded from client state)
    ...
    // Secure from header formatting: to prevent rejection by strict SMTP servers (e.g. mail.pixiatech.com, Gmail, Zoho)
    // the email address in From MUST match the authenticated SMTP user. We rewrite "Display Name <another@email>" 
    // to "Display Name <auth_user@email>" while preserving the friendly display name.
    let finalFrom = from;
    ...
    // Detect current client application origin
    const defaultUrl = `${req.secure ? 'https' : 'http'}://${req.headers.host}`;
    let cleanAppUrl = appUrl || defaultUrl;
    
    // Automatically rewrite development domain prefix to public pre-release prefix
    // to bypass Google Cloud Run IAM 403 blocks for external email client links.
    if (cleanAppUrl.includes('ais-dev-')) {
      cleanAppUrl = cleanAppUrl.replace('ais-dev-', 'ais-pre-');
    }
    
    const directConnectUrl = `${cleanAppUrl}?code=${code}`;
    const copyConnectUrl = `${cleanAppUrl}?code=${code}&copy=true`;

    // Compute dynamic emission and expiration times
    const now = new Date();
    const emissionTime = now.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit', second: '2-digit', timeZone: 'Europe/Paris' });
    const expirationDate = new Date(now.getTime() + 10 * 60 * 1000);
    const expirationTime = expirationDate.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit', second: '2-digit', timeZone: 'Europe/Paris' });

    try {
      const transporter = nodemailer.createTransport({
        host,
        port,
        secure: isSecure, // true inside 465, false for other TLS ports e.g. 587
        auth: {
          user,
          pass,
        },
        tls: {
          rejectUnauthorized: false, // ignores SSL certificate verification failures on custom ports
          minVersion: 'TLSv1.2'
        },
      ...
      // HTML body formatted to look EXACTLY like the "Client Webmail Sécurisé" simulator card
      const htmlContent = `
<!DOCTYPE html>
<html>
<head>
...
<body style="margin: 0; padding: 40px 10px; background-color: #f8fafc; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif;">
  <div style="max-width: 500px; margin: 0 auto; background-color: #ffffff; border: 1px solid #e2e8f0; border-radius: 24px; box-shadow: 0 10px 25px -5px rgba(0, 0, 0, 0.05), 0 8px 10px -6px rgba(0, 0, 0, 0.05); overflow: hidden;">
    ...
    <div style="padding: 40px 30px; text-align: center;">
              ...
              <table style="border-collapse: collapse; margin: 0 auto;">
                <tr>
                  <td style="padding: 0; vertical-align: middle; text-align: left;">
                    <!-- Beautiful Custom Simulated PixiaTech Circular Violet-Blue Logo -->
      ...
      <table style="margin: 0 auto 24px auto; text-align: center; border-collapse: collapse; width: 100%;">
        ...
        <tr>
          <td style="text-align: center;">
            <!-- Selectable styled single block mimicking 6 boxes visually but copyable as a whole without spaces -->
        ...
        <tr>
          <td style="text-align: center; padding-top: 2px;">
            <!-- Simulation Copy Code to Clipboard button inside genuine email -->
      ...
      <!-- Red security warning block card modeled exactly on the clean app style -->
