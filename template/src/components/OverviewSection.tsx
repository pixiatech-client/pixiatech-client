...
export const OverviewSection: React.FC<OverviewSectionProps> = ({
  ...
  const techTitle = lang === "FR" ? "TECHNOLOGIES EMBARQUÉES" : "TECHNOLOGIES INSIDE";
    ...
    ? [
        {
          num: "01",
          title: "ColdLED",
          desc: "La chaleur est l'ennemi principal des diodes à pas fin. ColdLED l'élimine, prolongeant la durée de vie et maintenant une luminosité éclatante là où les écrans conventionnels faiblissent.",
        },
        {
          num: "02",
          title: "SolidSkin",
          desc: "Le nano-revêtement protecteur SolidSkin et son indice IP65 avant protègent chaque micro-LED contre l'humidité, la poussière et les chocs, offrant un confort visuel sans reflets sur toute la surface.",
        },
      ]
    : [
        {
          num: "01",
          title: "ColdLED",
          desc: "Heat is the primary enemy of fine-pitch diodes. ColdLED eliminates it — extending lifespan and maintaining peak brightness where conventional displays fade.",
        },
        {
          num: "02",
          title: "SolidSkin",
          desc: "SolidSkin’s protective nano-layer and IP65 front rating shields every micro-LED against moisture, dust, and physical impact — delivering softer, smoother visuals across the entire display.",
        },
      ];
      ...
      <div className="wrap">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1.35fr_1fr] gap-8 lg:gap-24 items-end mb-14">
          ...
          <p className="text-[16.5px] leading-[1.65] text-[#4a4a46] m-0">
            {t.description.replace("PixiaTech", companyName)}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1.15fr_1fr] gap-6 items-stretch">
          ...
          <div className="relative bg-[#ebe8e1] h-[min(58vh,560px)] overflow-hidden border border-[#dad8d2]">
            <img
              ...
              loading="lazy"
              onError={(e) => {
                // Fallback to original remote URL if needed
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-[1px] bg-[#dad8d2] border border-[#dad8d2] mt-6">
          {stats.map((item) => (
            <div key={item.label} className="bg-[#f5f4f0] p-8">
              ...
              <div className="text-[12.5px] tracking-[0.02em] text-[#6b6a66] mt-3 leading-[1.5]">
                {item.label}
              </div>
            </div>
          ))}
