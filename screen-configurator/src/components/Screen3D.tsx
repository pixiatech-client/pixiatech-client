...
function Dalle({ position, args, texture, uvOffset, uvScale, isPlaying }: { position: [number, number, number], args: [number, number], texture: THREE.Texture | null, uvOffset: [number, number], uvScale: [number, number], isPlaying: boolean }) {
  ...
  const geometry = React.useMemo(() => {
    // We use BoxGeometry for depth
    const geo = new THREE.BoxGeometry(args[0], args[1], DEPTH);
    
    // UV modification for front face only to stretch a single video
    const uvs = geo.attributes.uv;
    // Front face in BoxGeometry is typically vertices 16-23
    // Indices: 0-3:right, 4-7:left, 8-11:top, 12-15:bottom, 16-19:front, 20-23:back
    for (let i = 16; i < 20; i++) {
        const u = uvs.getX(i);
        const v = uvs.getY(i);
  ...
  const materials = React.useMemo(() => {
    ...
    const ledMaterial = new THREE.MeshStandardMaterial({
      ...
      emissive: texture && isPlaying ? new THREE.Color(1, 1, 1) : new THREE.Color(0, 0, 0),
      emissiveMap: texture && isPlaying ? texture : null,
      emissiveIntensity: texture && isPlaying ? 1.4 : 0, 
      roughness: 0.4,
      metalness: 0,
      envMapIntensity: 0.1
    });
...
function HumanSilhouette({ position, isDarkMode = false }: { position: [number, number, number], isDarkMode?: boolean }) {
  ...
  return (
    <group position={position}>
      ...
      {/* Arms - Posed holding a tablet or just clean side pose */}
      ...
      {/* Dynamic Floor Shadow Disk (Additional realism) */}
...
function Screen({ width, height, isCurved, curveLeft, curveRight, isDarkMode }: Screen3DProps & { isDarkMode: boolean }) {
  ...
  const startVideo = React.useCallback(() => {
    ...
    // Prioritize direct link, then backup
    video.src = 'https://firebasestorage.googleapis.com/v0/b/studio-9205859220-a6440.firebasestorage.app/o/uploads%2F1765799832313_Devis%20Ecran.mp4?alt=media&token=99eec72d-0dab-4adb-bf36-061263381e09';
    video.loop = true;
    video.muted = true;
    ...
    const handleSuccess = () => {
      ...
      // Many video textures need flipY set to false to match Three.js UVs
      tex.flipY = true; 
      setTexture(tex);
      setIsPlaying(true);
      setHasError(false);
    ...
    video.play().catch((err) => {
      ...
      // If original fails, use a secondary reliable source
  ...
  const renderCurvedScreen = () => {
    ...
    // Initial angle pour le S - Doubled curvature effect as requested
  ...
  return (
    <group ref={meshRef}>
      ...
      {/* Human scale helper - visible in both modes with adaptive colors */}
...
export default function ScreenViewer(props: Screen3DProps) {
  ...
  // Robust Camera Reset/Adaptation Logic
  React.useEffect(() => {
    if (controlsRef.current) {
      // Calculate a zoom level based on the screen's size to keep it well-framed
      const maxDim = Math.max(props.width, props.height);
      const zoomBase = maxDim * 1.5;
      const heightOffset = props.height * 0.45;

      if (!isCurved) {
        controlsRef.current.object.position.set(zoomBase * 0.8, zoomBase * 0.5, zoomBase * 1.1);
        controlsRef.current.target.set(0, heightOffset, 0);
      } else {
        controlsRef.current.object.position.set(zoomBase * 0.9, zoomBase * 0.6, zoomBase * 1.3);
        controlsRef.current.target.set(0, heightOffset, 0);
      }
  ...
  const currentText = isDarkMode ? "text-slate-500" : "text-slate-400";
  ...
  return (
    <div className="w-full h-full rounded-[2.5rem] overflow-hidden relative shadow-inner group" style={{ background: currentEnv }}>
      ...
      {/* 3D Controls overlay - Dark Mode Pill Style */}
