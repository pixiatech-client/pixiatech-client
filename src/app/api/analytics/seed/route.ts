...
function makeSessionDay(dayOffset: number, startDay: number): number {
  const weekend = [0, 6].includes(new Date(startDay - dayOffset * DAY).getDay());
  const weekday = !weekend ? 15 + Math.floor(Math.random() * 12) : 8 + Math.floor(Math.random() * 7);
  const growth = 1 + (WINDOW_DAYS - dayOffset) / 160;
  return Math.max(3, Math.round(weekday * growth));
}
...
function makeSession(d: Date, visitor: SeededVisitor): AnalyticsSessionSummary {
  ...
  const cm: Record<string, string | undefined> = {
    ...
    social: 'social',
    referral: 'referral',
    campaign: Math.random() < 0.5 ? 'email' : 'cpc',
    other: undefined,
  };
  ...
  return {
    ...
    source,
    medium: cm[source],
    campaign: source === 'campaign' ? pick(['print-led', 'rentree-2026', 'noel-2025', 'led-week']) : undefined,
...
export async function POST() {
  try {
    ...
    for (let day = 0; day <= WINDOW_DAYS; day++) {
      ...
      const count = makeSessionDay(day, startDay);
    ...
    for (let i = 0; i < sessions.length; i += 450) {
      ...
      const chunk = sessions.slice(i, i + 450);
    ...
    for (let i = 0; i < messages.length; i += 450) {
      const batch = adminDb.batch();
      const chunk = messages.slice(i, i + 450);
