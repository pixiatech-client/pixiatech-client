...
export function NotificationBell({ isDark = false, userRole }: NotificationBellProps) {
  ...
  return (
    <div className="relative">
      ...
      >
        <Bell className={`w-5 h-5 transition-colors ${unreadCount > 0 ? 'text-red-500' : isDark ? 'text-gray-400 group-hover:text-rose-400' : 'text-gray-500 group-hover:text-rose-500'} ${unreadCount > 0 ? 'animate-wiggle' : ''}`} />
        {unreadCount > 0 && (
        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed right-4 md:right-4 top-20 mt-3 w-[calc(100vw-2rem)] md:w-[360px] bg-white dark:bg-zinc-900 rounded-[24px] shadow-[0_20px_50px_rgba(0,0,0,0.1)] border border-gray-100 dark:border-white/5 z-[9999] overflow-hidden"
            >
