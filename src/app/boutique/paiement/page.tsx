...
import { ShoppingBag, Lock, Shield, Check, CreditCard, Wallet, MapPin, User, ChevronDown, Tag, X, Info, Building2, ArrowLeft, ArrowRight, Mail, Loader2, Send, LogIn } from 'lucide-react';
...
export default function CheckoutPage() {
  ...
  // Connexion facultative rattachée au checkout (client non connecté).
  // Le client peut toujours continuer en invité sans ouvrir ce panneau.
  ...
  // Connexion par lien magique depuis le checkout : réutilise exclusivement
  // /api/boutique/send-magic-link avec l'email actuellement saisi dans le formulaire.
  // La réponse est générique (anti-énumération) ; l'utilisateur ne sait jamais si
  // le compte existe, la possession de la boîte mail fait foi.
  async function handleCheckoutMagicLink(e: React.FormEvent) {
    e.preventDefault();
    const email = delivery.email.trim();
    if (!email) {
      setLoginError('Veuillez renseigner votre adresse email ci-dessus.');
      return;
    }
  ...
  // Connexion par mot de passe depuis le checkout : réutilise exclusivement
  // /api/boutique/login-password. L'email est repris du formulaire ; seul le mot
  // de passe est saisi. En cas de succès on recharge la page : client_session est
  // posée en cookie, puis session-status + le pré-remplissage de la Tâche 2
  // réinjectent automatiquement le profil (le panier persiste en localStorage).
  async function handleCheckoutPasswordLogin(e: React.FormEvent) {
    e.preventDefault();
    const email = delivery.email.trim();
    if (!email || !loginPassword || loginLoading) return;
    setLoginError('');
                        ...
                        <div className="mt-4 pt-4 border-t border-gray-100">
                          {!loginPanelOpen ? (
                            ...
                            <div className="bg-gray-50/70 border border-gray-200 rounded-xl p-4">
                              ...
                              {loginDisplay === 'magic' && (
                                <form onSubmit={handleCheckoutMagicLink} className="space-y-3">
                                  <div className="text-[12px] font-semibold text-gray-700">Recevoir un lien de connexion</div>
                                  <p className="text-[11px] text-gray-500 leading-relaxed">
                                    {delivery.email
                                      ? <span>Un lien vous sera envoyé à <span className="font-semibold text-gray-700">{delivery.email}</span>.</span>
                                      : 'Renseignez votre email ci-dessus pour recevoir un lien.'}
                                  </p>
                                  {loginMagicSent ? (
                                    ...
                                    <button
                                      type="submit"
                                      disabled={loginMagicSending || !delivery.email.trim()}
                              ...
                              {loginDisplay === 'password' && (
                                <form onSubmit={handleCheckoutPasswordLogin} className="space-y-3">
