...
export default function CheckoutFlow() {
  ...
  async function handleRegister(e: React.FormEvent) {
    ...
    (['firstName', 'lastName', 'email', 'phone', 'addressLine1', 'city', 'postcode', 'country'] as CustomerInfoField[]).forEach(f => {
      const err = validateCustomerField(f, formValues[f]);
      newTouched[f] = true;
      if (err) newErrors[f] = err;
    });

    if (!formValues.firstName.trim()) newErrors.firstName = 'Veuillez saisir votre prénom.';
    if (!formValues.lastName.trim()) newErrors.lastName = 'Veuillez saisir votre nom.';
    if (!formValues.email.trim()) newErrors.email = 'Veuillez saisir une adresse e-mail.';
    if (!formValues.phone.trim()) newErrors.phone = 'Veuillez saisir votre numéro de téléphone.';
    if (!formValues.addressLine1.trim()) newErrors.addressLine1 = 'Merci de saisir une adresse complète.';
    if (!civility) newErrors.civility = 'Veuillez sélectionner votre civilité.';

    if (!registerPassword) {
      newErrors.registerPassword = 'Veuillez créer un mot de passe.';
    } else if (registerPassword.length < 8) {
      newErrors.registerPassword = 'Le mot de passe doit contenir au moins 8 caractères.';
    }
    if (!acceptTerms) newErrors.terms = 'Veuillez accepter les conditions générales de vente.';
  ...
  async function persistAddress(): Promise<boolean> {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        throw new Error(data?.error || "Erreur lors de l'enregistrement de votre adresse.");
      }
    ...
    >
      <div className={cn('mx-auto flex min-h-screen w-full flex-col px-5 py-8 sm:px-8 sm:py-10', step === 'delivery' ? 'max-w-6xl' : 'max-w-5xl')}>
        <div className="mb-10 flex items-center justify-between">
          ...
          <nav className="flex items-center justify-center gap-1 sm:gap-2" aria-label="Étapes de la commande">
            {FLOW_STEPS.map((s, i) => {
              ...
              return (
                <div key={s.id} className="flex items-center gap-1 sm:gap-2">
                  <div className={cn('flex h-8 w-8 items-center justify-center rounded-full text-xs font-bold transition-colors', stateClass)}>
                    {done ? <CheckCircle2 size={15} /> : i + 1}
                  </div>
                  <span className={cn('text-xs font-semibold sm:text-sm', active ? 'text-gray-900' : 'text-gray-400')}>
                    {s.label}
                  </span>
                  {i < FLOW_STEPS.length - 1 && <span className="mx-1 h-px w-8 bg-gray-300 sm:w-12" />}
                </div>
              );
            })}
          </nav>
          ...
          >
            {step === 'auth' && !sessionReady && (
              <div className="flex flex-1 flex-col items-center justify-center py-24 text-center">
                ...
                <p className="mt-4 text-sm text-gray-400">Chargement de votre session…</p>
              </div>
            )}
              ...
              <div className="flex flex-1 flex-col">
                <div className="text-center">
                  ...
                  <p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-gray-500 sm:text-base">
                    Identifiez-vous pour récupérer automatiquement vos informations et accélérer votre commande.
                  </p>
                </div>
                ...
                <div className="mt-10 grid grid-cols-1 gap-6 md:grid-cols-2">
                  <section className="flex flex-col rounded-2xl border border-gray-200/80 bg-white p-6 shadow-sm sm:p-8">
                    {loginMode === 'login' ? (
                      <>
                        <h2 className="text-center text-xl font-bold text-gray-900">Se connecter</h2>
                        <p className="mt-1.5 text-center text-sm leading-6 text-gray-500">
                          Retrouvez vos informations client et votre historique de commandes.
                        </p>
                      ...
                      <>
                        ...
                        <form onSubmit={handleForgotPassword} className="mt-6 flex flex-1 flex-col gap-4">
                          <p className="text-xs leading-5 text-gray-500">
                            Saisissez l&apos;adresse email associée à votre compte. Un lien de connexion sécurisé
                            vous sera envoyé.
                          </p>
                          <div>
                          ...
                          {forgotSent ? (
                            <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-xs leading-5 text-emerald-700">
                              Si un compte correspond à cette adresse, un lien de connexion vous a été envoyé.
                              Vérifiez vos spams.
                            </div>
                          ) : (
                            <button
                  ...
                  <section className="flex flex-col rounded-2xl border border-gray-200/80 bg-white p-6 shadow-sm sm:p-8">
                    <h2 className="text-center text-xl font-bold text-gray-900">Créer votre compte</h2>
                    <p className="mt-1.5 text-center text-sm leading-6 text-gray-500">
                      Renseignez vos informations pour créer votre compte client PIXIATECH.
                    </p>
                    <div className="mt-6 flex flex-1 flex-col items-center rounded-2xl border border-gray-100 bg-gray-50 p-6">
                      ...
                      <h3 className="mt-3 text-sm font-bold text-gray-900">Nouveau client ?</h3>
                      <p className="mt-1.5 max-w-[16rem] text-center text-xs leading-5 text-gray-500">
                        Créez votre compte en quelques instants. Votre adresse sera pré-remplie pour vos
                ...
                <p className="mt-8 flex items-center justify-center gap-1.5 text-xs leading-4 text-gray-400">
                  ...
                  Vos informations sont protégées et ne sont jamais partagées.
                </p>
              </div>
            )}
            ...
            {step === 'register' && (
              <div className="flex flex-1 flex-col items-center pb-16">
                <div className="text-center">
                  ...
                  <p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-gray-500 sm:text-base">
                    Renseignez vos informations pour créer votre compte client PIXIATECH et accélérer vos
                    prochaines commandes.
                  </p>
                </div>
                  ...
                  >
                    <div>
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {(['Monsieur', 'Madame', 'Mademoiselle'] as const).map(option => (
                          <button
                            key={option}
                            type="button"
                            onClick={() => { setCivility(option); setError(''); }}
                            className={cn(
            ...
            {step === 'address' && (
              <div className="flex flex-1 flex-col items-center pb-16">
                <div className="w-full max-w-2xl text-center">
                  ...
                  <p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-gray-500 sm:text-base">
                    Vérifiez que les informations ci-dessous sont correctes avant de continuer.
                  </p>
                </div>
                  ...
                  {addressEditing ? (
                    <form onSubmit={(e) => { e.preventDefault(); persistAddress(); }} noValidate>
                      <div className="mb-6 flex items-center justify-between">
                        <h2 className="text-lg font-bold text-gray-900">Modifier mon adresse</h2>
                        <button
                          type="button"
                    ...
                    <div>
                      ...
                      {companyName.trim() && (
                        <div className="border-b border-gray-100">
                          <RecapRow icon={<Building2 size={16} />} label="Entreprise" value={companyName.trim()} />
                        </div>
                      )}
                      <div className="border-b border-gray-100">
                        <RecapRow icon={<MapPin size={16} />} label="Adresse" value={formValues.addressLine1 || '—'} />
                      </div>
                      {formValues.addressLine2.trim() && (
                        <div className="border-b border-gray-100">
                          <RecapRow icon={<MapPin size={16} />} label="Complément d&apos;adresse" value={formValues.addressLine2.trim()} />
                        </div>
                      )}
                      <div className="border-b border-gray-100">
                        <RecapRow
                      ...
                      <div className="border-b border-gray-100">
                        <RecapRow icon={<Globe size={16} />} label="Pays" value={countryLabel} />
                      </div>
                      <div className="border-b border-gray-100">
                        <RecapRow icon={<Phone size={16} />} label="Téléphone" value={formValues.phone || '—'} />
                      </div>
                      <div className="border-b border-gray-100">
                        <RecapRow icon={<Mail size={16} />} label="E-mail" value={formValues.email || '—'} />
                      </div>
                    </div>
                  )}
                </div>
