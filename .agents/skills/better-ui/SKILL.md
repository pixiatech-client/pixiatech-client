...
| Animations | Interruptible transitions, scale on press, skipping animation on page load, theme switching, motion restraint |
| Enter & Exit | Staged entrances, stagger timing, exit transitions |
| Icon Transitions | Cross-fading an icon on state change, with and without a motion library |
...
A subtle `scale(0.96)` on click gives buttons tactile feedback. Always use `0.96`; anything below `0.95` feels exaggerated. Add a `static` prop to disable it when motion would be distracting.
...
A theme flip changes color, background, border, and shadow on nearly every element at once, so every transition on those properties fires together and the switch smears instead of snapping. Inject `*,*::before,*::after{transition:none !important}`, force a reflow, then remove it on the next frame.
