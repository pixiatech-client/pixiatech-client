...
export const HeroSection: React.FC<HeroSectionProps> = ({
  ...
  const title = cmsHero.title || initialTitle;
  const subtitle = cmsHero.subtitle || (lang === "FR" ? "Écran LED fin pitch intérieur" : "Indoor LED display");
  const primaryCta = cmsHero.primaryCta || t.ctaQuote;
  const secondaryCta = cmsHero.secondaryCta || t.ctaSpecs;
  const heroMedia = cmsHero.heroImage || cmsHero.image;

  return (
    <EditableWrapper sectionKey="hero" sectionLabel="Hero PXT Fine (Titre, CTA, Médias)">
      <section
        id="hero"
      ...
      >
        <div className="wrap">
          ...
          <div className="flex justify-between items-end gap-8 flex-wrap mt-8">
            <div>
              <div
                className="text-[clamp(20px,1.9vw,28px)] font-bold mb-3 text-[#f5f4f0]"
                style={cmsHero.titleFontSize ? { fontSize: `${cmsHero.titleFontSize}px` } : {}}
              >
                {subtitle}
              </div>
          ...
          <div className="mt-8 relative overflow-hidden bg-[#0a0a0a] border border-[#1a1a1a] shadow-2xl">
            <div className="relative w-full h-[min(64vh,640px)] flex items-center justify-center">
              {heroMedia && !heroMedia.endsWith(".webm") && !heroMedia.endsWith(".mov") && !heroMedia.endsWith(".mp4") ? (
                <img
                  src={heroMedia}
                  alt={title}
                ...
                <video
                  ...
                  playsInline
                  preload="metadata"
                  poster={heroMedia || "/uploads/products/wp/wp-cabinet.jpg"}
