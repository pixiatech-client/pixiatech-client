...
export const ShowreelSection: React.FC<ShowreelSectionProps> = ({
  ...
  const handleFileProcess = (file: File) => {
    if (!file || !file.type.startsWith("image/")) {
      setUploadNotice(
        lang === "FR"
          ? "Veuillez sélectionner un fichier image valide (.webp, .png, .jpg)"
          : "Please select a valid image file (.webp, .png, .jpg)"
      );
      return;
    }
    setIsUploading(true);
    setUploadNotice(
      lang === "FR"
        ? "Intégration de votre visuel original en cours..."
        : "Uploading your original visual..."
    );
    ...
    reader.onload = async (e) => {
      try {
        ...
        const res = await fetch("/api/upload-showreel", {
          method: "POST",
        ...
        if (data.success) {
          ...
          setUploadNotice(
            lang === "FR"
              ? "✓ Votre visuel original est intégré avec succès sur l'écran LED !"
              : "✓ Your original visual is now live on the LED screen!"
          );
          reloadImageRef.current();
          setTimeout(() => setUploadNotice(null), 4000);
        } else {
          setIsUploading(false);
          setUploadNotice("Erreur : " + data.error);
        }
      } catch (err: any) {
        setIsUploading(false);
        setUploadNotice("Erreur lors de l'enregistrement de l'image");
      }
    };
  ...
  const t = {
    ...
    scrollHint:
      lang === "FR"
        ? "FAITES DÉFILER — DU PIXEL À LA VILLE"
        : "SCROLL TO PULL BACK — ONE PIXEL TO THE CITY",
    eyebrow:
      lang === "FR"
        ? "02 / SHOWREEL — DU PIXEL À LA VILLE"
        : "02 / SHOWREEL — ONE PIXEL TO THE CITY",
      ...
      lang === "FR"
        ? [
            [
              ...
              "Un module calibré.",
              "Chaque LED est mesurée et appairée en laboratoire afin d'assurer une chromaticité et une luminance parfaites d'un bord à l'autre.",
            ],
            [
              "02 / LE MUR",
              "Les cabinets s'assemblent en",
              "une surface continue.",
              "Châssis 500 × 500 mm, profondeur 29.5 mm. Aucune limite de largeur ou de hauteur.",
            ],
            [
              "03 / L'ARCHITECTURE",
              "L'écran devient",
              "la façade.",
              "Nous n'intégrons pas simplement des écrans dans l'espace : nous faisons des écrans une partie intégrante de l'architecture.",
            ],
            [
              "04 / LA VILLE",
              "De la technologie",
              "à la skyline métropolitaine.",
              "",
            ],
          ]
        : [
            [
              "01 / THE MODULE",
              "Thousands of pixels.",
              "One calibrated module.",
              "Every LED measured and matched, so no module is brighter than its neighbour.",
            ],
            [
              "02 / THE WALL",
              "Cabinets lock into",
              "one seamless surface.",
              "500 × 500 mm cabinets, 29.5 mm deep. Any width. Any height.",
            ],
            [
              "03 / THE ARCHITECTURE",
              "The screen becomes",
              "the façade.",
              "We don't place screens into spaces. We make screens part of the space.",
            ],
            [
              "04 / THE CITY",
              "From engineering",
  ...
  useEffect(() => {
    ...
    // Procedural city building generator
    interface Building {
      x: number;
      z: number;
      w: number;
    ...
    // Billboard content canvas (1536 x 896)
    ...
    const renderFrame = (timestamp: number) => {
      ...
      const s = isSettled ? scrollProgressTarget : scrollProgressCur;
      ...
      // Camera distance exponential mapping (0.02 mm to ~900 m)
      const camDist = 0.02 * Math.pow(10, 4.65 * s);
      ...
      const project = (pX: number, pY: number, pZ: number) => {
        ...
        return {
          x: viewW / 2 + ((pX - camOffsetX) * projScale) / depth,
          y: horizonY - ((pY - camHeight) * projScale) / depth,
          dz: depth,
        };
      };
      ...
      // Initialize billboard content texture if needed
      ...
      // Distant sky & atmosphere when pulled back
      if (camDist > 8) {
      ...
      // Draw perspective quad helper
      const quad = (p1: { x: number; y: number }, p2: { x: number; y: number }, p3: { x: number; y: number }, p4: { x: number; y: number }) => {
        ctx.beginPath();
      ...
      // City buildings rendering at high camera distances
      if (camDist > 12) {
        ...
        for (const { b, z0 } of visibleBuildings) {
          ...
          // Rooftop communications antenna & beacon
          ...
          // Exterior LED screen on building
      ...
      if (camDist > 30) {
        ...
        ctx.drawImage(shadowGradCanvas, mainBL.x, mainBL.y - (mainBL.y - mainTR.y) * 0.1, mainBR.x - mainBL.x, (mainBL.y - mainTR.y) * 0.1);
      } else {
      ...
      // CLOSE-UP STAGE (Subpixel SMD LED Packages)
      if (pixelScreenScale >= 26) {
        ctx.fillStyle = "#070707";
        ctx.fillRect(0, 0, viewW, viewH);

        // Pre-render diode package cache
        const getDiodeCache = (sz: number) => {
          ...
          drawRoundedRect(dCtx, roundedSz / 2 - 0.4 * innerSz, roundedSz / 2 - 0.32 * innerSz, 0.8 * innerSz, 0.64 * innerSz, 0.04 * innerSz);
          dCtx.fill();
      ...
      // MEDIUM ZOOM (Locked Cabinets Matrix)
      ...
      // DISTANT VIEW (Monolithic Architectural Wall)
      else {
      ...
      // Ground Reflection & Night Atmosphere
      if (camDist > 20) {
      ...
      // Animated chapter text switches
    ...
    const onScroll = () => {
      ...
      scrollProgressTarget = clamp(-rect.top / (rect.height - viewH), 0, 1);
      ...
      >
        ...
        {isDragging && (
          ...
          >
            ...
            <div style={{ textAlign: "center" }}>
              <div style={{ color: "#F5F4F0", fontWeight: 700, fontSize: 18, marginBottom: 6 }}>
                {lang === "FR" ? "Déposez votre image ici" : "Drop your image here"}
              </div>
              <div style={{ color: "#C3F910", fontFamily: "monospace", fontSize: 12, letterSpacing: ".1em" }}>
                {lang === "FR"
                  ? "APPLICATION DIRECTE SUR L'ÉCRAN LED DU SHOWREEL"
                  : "DIRECT APPLICATION ON SHOWREEL LED SCREEN"}
              </div>
            </div>
          </div>
        )}
        ...
        {uploadNotice && (
          <div
            style={{
              ...
              padding: "10px 20px",
              borderRadius: 6,
              boxShadow: "0 0 30px rgba(195,249,16,0.3)",
              zIndex: 1000,
              display: "flex",
              alignItems: "center",
              gap: 10,
          ...
          >
            <span style={{ color: "#C3F910", fontSize: 14 }}>●</span>
            {uploadNotice}
          </div>
        )}
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            <span>{t.label}</span>
            {/* Custom Visual Uploader Button */}
            <div style={{ pointerEvents: "auto" }}>
              <button
                type="button"
                onClick={() => fileInputRef.current?.click()}
                disabled={isUploading}
                title={lang === "FR" ? "Importer votre image originale (.webp, .png, .jpg)" : "Upload your original image"}
                style={{
                  display: "inline-flex",
                  alignItems: "center",
                  gap: 7,
                  background: "rgba(20,20,19,0.85)",
                  border: "1px solid rgba(195,249,16,0.35)",
                  color: "#F5F4F0",
                  fontSize: 10,
                  letterSpacing: ".12em",
                  fontFamily: "monospace",
                ...
                onMouseLeave={(e) => {
                  e.currentTarget.style.borderColor = "rgba(195,249,16,0.35)";
                  e.currentTarget.style.background = "rgba(20,20,19,0.85)";
              ...
              >
                ...
                {isUploading
                  ...
                  : (lang === "FR" ? "IMPORTER MON VISUEL (.WEBP / .PNG)" : "UPLOAD CUSTOM VISUAL")}
              </button>
              <input
                ref={fileInputRef}
                type="file"
