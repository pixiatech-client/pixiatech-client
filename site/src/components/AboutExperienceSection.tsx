...
export const AboutExperienceSection: React.FC<AboutExperienceSectionProps> = ({
  ...
  const t = {
    ...
    title: lang === "FR" ? "La lumière prend tout son sens lorsque vous vous tenez devant elle." : "Pixels make more sense when you stand in front of them.",
    desc:
      lang === "FR"
    ...
    visitBtn: lang === "FR" ? "Visiter le PIXIATECH Experience Center →" : "Visit PIXIATECH Experience Center →",
    ...
    processTitle: lang === "FR" ? "De l'esquisse architecturale à la mise en service." : "From initial survey to final commissioning.",
    steps: [
      {
        num: "01",
        title: lang === "FR" ? "Étude & Métrologie" : "Survey & Site Metrology",
        desc: lang === "FR" ? "Analyse structurelle, étude de charge statique et dynamique, calcul des distances de recul optimales et bilan photométrique précis." : "Structural load study, viewing distance analysis, optical and power budgeting.",
      },
      {
        num: "02",
        title: lang === "FR" ? "Modélisation 3D BIM" : "3D BIM Integration",
        desc: lang === "FR" ? "Conception des sous-structures mécano-soudées sur mesure et intégration parfaite aux maquettes numériques du bâtiment." : "Custom sub-structure engineering and full architectural BIM coordination.",
      },
      {
        num: "03",
        title: lang === "FR" ? "Appairage en Salle Blanche" : "Cleanroom Binning",
        desc: lang === "FR" ? "Mesure spectrale individuelle de chaque lot de LED pour une cohérence colorimétrique sub-deltaE sur l'ensemble du mur." : "Individual spectral binning of every LED batch for absolute color uniformity.",
      },
      {
        num: "04",
        title: lang === "FR" ? "Installation & Calibration" : "Deployment & Calibration",
        desc: lang === "FR" ? "Montage certifié par nos ingénieurs et calibration radiométrique sur site avec les processeurs NovaStar COEX / Brompton." : "Certified installation and full radiometric field calibration.",
      },
      {
        num: "05",
        title: lang === "FR" ? "Télé-Surveillance 24/7" : "24/7 Telemetry & Warranty",
        desc: lang === "FR" ? "Diagnostic continu de chaque cabinet via cloud sécurisé, pièces de rechange dédiées et garantie complète de 5 ans." : "Continuous cabinet-level health monitoring and comprehensive 5-year warranty.",
      },
    ],
  };
    ...
    <>
      ...
      <section className="relative min-h-[580px] md:min-h-[720px] bg-[#000] border-t border-[#1f1f1f] flex items-end overflow-hidden">
        ...
        <div className="relative z-10 wrap pb-20 pt-36">
          ...
          <div className="text-[11.5px] font-mono tracking-[0.2em] text-[#7a7a76] uppercase">
            {t.location}
          </div>
        </div>
      </section>
      ...
      <section id="process" className="theme-dark bg-[#080808] border-t border-[#1f1f1f] py-24 md:py-32">
        <div className="wrap">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-px bg-[#1f1f1f] border border-[#1f1f1f]">
            {t.steps.map((st) => (
              <div key={st.num} className="bg-[#0b0b0a] p-7 flex flex-col justify-between hover:bg-[#111110] transition-colors">
                <div>
                  <div className="text-[12px] font-mono tracking-[0.2em] text-[#C3F910] font-bold mb-4">
                    {st.num}
                  </div>
                ...
                <p className="text-[13px] text-[#8a8a86] leading-relaxed mt-4">
                  {st.desc}
                </p>
              </div>
            ))}
